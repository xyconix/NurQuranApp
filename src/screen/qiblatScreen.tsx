...
import { View, Text, Image, StyleSheet, PermissionsAndroid, Platform, SafeAreaView, TouchableOpacity } from 'react-native';
...
const QiblaScreen = () => {
  ...
  const calculateQibla = (lat: number, lon: number) => {
    ...
    const angle = Math.atan2(
      ...
      Math.cos(phiU) * Math.tan(phiK) - Math.sin(phiU) * Math.cos(lambdaK - lambdaU)
    );
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.compassSection}>
        ...
        <View style={styles.outerCircle}>
          ...
          <View style={styles.innerCircle}>
            ...
            <View style={[styles.pointerContainer, { transform: [{ rotate: `${qiblaRotation}deg` }] }]}>
      ...
      <View style={styles.instructionCard}>
        ...
        <Text style={styles.instructionText}>• Allow location and compass access</Text>
        <Text style={styles.instructionText}>• Rotate yourself until the arrow points up</Text>
        <Text style={styles.instructionText}>• You are now facing the Qibla directions</Text>
      </View>
    </SafeAreaView>
  );
};
...
const styles = StyleSheet.create({
  ...
  cardinalW: { position: 'absolute', left: -30, color: 'white', fontWeight: 'bold' },
  cardinalE: { position: 'absolute', right: -30, color: 'white', fontWeight: 'bold' },
  centerDot: {
    width: 80,
    height: 80,
    borderRadius: 40,
